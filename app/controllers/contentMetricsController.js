...
const createNewContentEvent = async (req, res, next) => {
  const { action, tier, genre, userUId, albumId, albumName, songId, songName } =
    req.body;
...
const aggregateBySongsListened = async (req, res, next) => {
  try {
    ...
    console.error(e);
    next(
      ApiError.internalError("Internal error when aggregating listened songs")
    );
  }
};
...
const aggregateByAlbumsListened = async (req, res, next) => {
  try {
    ...
    console.error(e);
    next(
      ApiError.internalError("Internal error when aggregating listened albums")
    );
  }
};
...
module.exports = {
  ...
  aggregateBySongsListened,
  aggregateByAlbumsListened,
  aggregateByGenresListened,
};
